...
from collect_live_kalshi_ws import load_local_env, load_private_key
...
class KalshiClient:
    def __init__(self, authenticated: bool):
        ...
        if authenticated:
            ...
            if self.key_id:
                self.private_key = load_private_key(argparse.Namespace(private_key_path=None))
    ...
    def paginate(
        self, path: str, params: dict, field: str, auth: bool = False,
        row_filter=None, progress_label: str | None = None,
    ):
        ...
        while True:
            ...
            rows.extend(row for row in page_rows if row_filter is None or row_filter(row))
            pages += 1
